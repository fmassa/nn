...
void THNN_(SpatialMaxPooling_updateOutput)(THNNState* state,
    THTensor* input,
    THTensor* output,
    int kW, int kH,
    int dW, int dH,
    ...
    THTensor* indices)
{
  ...
  if (!(input->size[dimw] >= kW - padW && input->size[dimh] >= kH - padH))
...
void THNN_(SpatialMaxPooling_updateGradInput)(THNNState* state,
    THTensor* input,
    THTensor* gradOutput,
    THTensor* gradInput,
    int dW, int dH,
