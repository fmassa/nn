...
{
  THArgCheck(input->nDimension == 4 || input->nDimension == 5, 2, "4D or 5D (batch-mode) tensor expected");
  int dimt = 1;
  int dimh = 2;
  int dimw = 3;
...
)
{
  ...
  THArgCheck(nOutputPlane == gradOutput->size[dimPlane], 9, 
