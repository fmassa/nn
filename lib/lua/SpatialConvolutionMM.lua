...
function SpatialConvolutionMM:updateOutput(input)
   ...
   THNN.errcheck(
      'THNN_RealSpatialConvolution_updateOutput',
      input:type(),
      THNN.NULL,
      input:cdata(),
      self.output:cdata(),
...
function SpatialConvolutionMM:updateGradInput(input, gradOutput)
   if self.gradInput then
      ...
      if input:dim() == 3 then
        input = input:view(1,input:size(1),input:size(2),input:size(3))
        gradOutput = gradOutput:view(1,gradOutput:size(1),gradOutput:size(2),gradOutput:size(3))
        is_batch = false
      end

      THNN.errcheck(
         'THNN_RealSpatialConvolution_updateGradInput',
         input:type(),
         THNN.NULL,
         input:cdata(),
         gradOutput:cdata(),
      ...
      if not is_batch then
        self.gradInput:resize(self.gradInput:size(2),
           self.gradInput:size(3),self.gradInput:size(4))
      end
...
function SpatialConvolutionMM:accGradParameters(input, gradOutput, scale)
   ...
   THNN.errcheck(
      'THNN_RealSpatialConvolution_accGradParameters',
      input:type(),
      THNN.NULL,
      input:cdata(),
      gradOutput:cdata(),
